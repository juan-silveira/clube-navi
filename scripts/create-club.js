 ...
 * Create Club Script
 *
 * Cria um novo club com:
 * - Registro no Master DB
 * - Criação do database do club
 * - Execução de migrations
 * - Configuração inicial (branding, módulos, cashback, etc.)
 *
 * Usage:
 *   node scripts/create-club.js --slug=empresa-a --name="Empresa A" --email=admin@empresa-a.com
 */
...
async function createDatabase(dbName, dbUser, dbPassword) {
  ...
  try {
    ...
    // NOTA: Em ambiente de desenvolvimento, estamos usando o mesmo usuário para todos os clubs
    // Em produção, cada club deve ter seu próprio usuário PostgreSQL
    // Por enquanto, todos os clubs usarão clube_digital_user com databases separados
...
/**
 * Executa migrations do club
 */
async function runTenantMigrations(dbUrl) {
  console.log('\n🔄 Running club migrations...');

  try {
    // Set temp env var
    process.env.CLUB_DATABASE_URL = dbUrl;

    // Run migrations (usando schema do diretório club/)
...
/**
 * Cria configurações iniciais do club
 */
async function createTenantConfigs(masterPrisma, clubId) {
  console.log('\n⚙️  Creating club configurations...');

  try {
    // 1. Configuração de Cashback (padrão)
    await masterPrisma.clubCashbackConfig.create({
      data: {
        clubId,
    ...
    await masterPrisma.clubWithdrawalConfig.create({
      data: {
        clubId,
    ...
    for (const module of defaultModules) {
      await masterPrisma.clubModule.create({
        data: {
          clubId,
 ...
 */
async function main() {
  console.log('🚀 Clube Digital - Create Club Script\n');
  ...
  if (!args.slug || !args.name || !args.email) {
    ...
    console.log('\nUsage:');
    console.log('  node scripts/create-club.js --slug=empresa-a --name="Empresa A" --email=admin@empresa-a.com');
    console.log('\nRequired:');
    console.log('  --slug        Slug único do club (ex: empresa-a)');
  ...
  try {
    // 1. Verificar se club já existe
    console.log(`🔍 Checking if club "${args.slug}" already exists...`);

    const existing = await masterPrisma.club.findUnique({
      where: { slug: args.slug }
    });

    if (existing) {
      console.error(`❌ Club with slug "${args.slug}" already exists!`);
      process.exit(1);
    }
    ...
    // Em desenvolvimento, usar o mesmo usuário para todos os clubs
    ...
    // 4. Criar club no Master DB
    console.log('\n💾 Creating club in Master DB...');
    ...
    console.log('\n📋 Club Information:');
    console.log(`  Slug:          ${club.slug}`);
    console.log(`  Company:       ${club.companyName}`);
    console.log(`  Subdomain:     ${club.subdomain}.api.clubedigital.com.br`);
    console.log(`  Status:        ${club.status}`);
    console.log(`  Plan:          ${club.subscriptionPlan}`);
    ...
    console.log('  2. Build mobile apps for this club');
