...
const ApiTab = ({ settings, handleInputChange, handleToggle }) => {
  const { t } = useTranslation('systemSettings');

  return (
    <Card title={t('api.sectionTitle')} icon="heroicons-outline:server">
      <div className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <Textinput
            label={t('api.apiVersion')}
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between p-4 border rounded-lg">
            <div>
              <h4 className="font-medium text-gray-900 dark:text-white">{t('api.enablePublicApi')}</h4>
              <p className="text-sm text-gray-600 dark:text-gray-400">
                {t('api.enablePublicApiDescription')}
              </p>
            </div>
          ...
          <div className="flex items-center justify-between p-4 border rounded-lg">
            <div>
              <h4 className="font-medium text-gray-900 dark:text-white">{t('api.enableWebhooks')}</h4>
              <p className="text-sm text-gray-600 dark:text-gray-400">
                {t('api.enableWebhooksDescription')}
              </p>
            </div>
            <label className="relative inline-flex items-center cursor-pointer">
              <input
          ...
          <div className="flex items-center justify-between p-4 border rounded-lg">
            <div>
              <h4 className="font-medium text-gray-900 dark:text-white">{t('api.enableCors')}</h4>
              <p className="text-sm text-gray-600 dark:text-gray-400">
                {t('api.enableCorsDescription')}
              </p>
            </div>
            <label className="relative inline-flex items-center cursor-pointer">
              <input
