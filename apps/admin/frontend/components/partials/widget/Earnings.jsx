    ...
    <Card
      ...
      headerslot={
        <div className="flex flex-col items-end">
          <span className="text-sm text-slate-500 dark:text-slate-400">{t('earnings.accumulatedEarnings')}</span>
            ...
            <thead className="bg-slate-50 dark:bg-slate-800">
              <tr>
                <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                  {t('earnings.table.token')}
                </th>
                <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                  {t('earnings.table.amount')}
                </th>
                <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                  {t('earnings.table.valueInBRL')}
                </th>
                <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                  {t('earnings.table.date')}
                </th>
              </tr>
            </thead>
                ...
                <tr>
                  <td colSpan={4} className="px-4 py-8 text-center text-slate-500 dark:text-slate-400">
                    <div className="flex flex-col items-center">
                      ...
                      <span className="font-medium">{t('earnings.loading')}</span>
                      <span className="text-sm">{t('earnings.loadingSubtitle')}</span>
                    </div>
                  </td>
                </tr>
              ) : error ? (
                <tr>
                  <td colSpan={4} className="px-4 py-8 text-center text-slate-500 dark:text-slate-400">
                    <div className="flex flex-col items-center">
                      ...
                      <span className="font-medium text-red-600">{t('earnings.error')}</span>
                ...
                <tr>
                  <td colSpan={4} className="px-4 py-8 text-center text-slate-500 dark:text-slate-400">
                    <div className="flex flex-col items-center">
                      ...
                      <span className="font-medium">{t('earnings.noResults')}</span>
                      <span className="text-sm">{t('earnings.noResultsSubtitle')}</span>
                    </div>
                  </td>
                </tr>
              )}
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between space-y-3 md:space-y-0">
              <div className="text-sm text-slate-500 dark:text-slate-400 text-center md:text-left">
                {t('transactions.pagination.showing', {
                  from: startIndex + 1,
                  to: Math.min(startIndex + pagination.limit, pagination.total),
