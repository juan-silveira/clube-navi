...
const useTokenMask = (initialValue = '') => {
  ...
  const getNumericValue = useCallback((val = value) => {
    ...
    if (stringVal.includes(',')) {
      ...
      // Se tem ponto, assumir que é sempre separador de milhares brasileiro
      // porque o hook formata automaticamente usando vírgula para decimais
      if (cleanValue.includes('.')) {
        const parts = cleanValue.split('.');

        // Se tem exatamente 3 dígitos após o ponto, é separador de milhares
        // Ex: "1.000", "4.300", "10.500"
        if (parts.length === 2 && parts[1].length === 3) {
          // É separador de milhares: remover ponto e interpretar como inteiro
          const onlyDigits = cleanValue.replace(/\./g, '');
          const result = parseInt(onlyDigits);
          return isNaN(result) ? 0 : result;
        } else if (parts.length > 2) {
          // Múltiplos pontos (ex: "1.234.567") - remover todos e tratar como milhares
          const onlyDigits = cleanValue.replace(/\./g, '');
          const result = parseInt(onlyDigits);
          return isNaN(result) ? 0 : result;
        } else if (parts.length === 2 && parts[1].length <= 2) {
          // Menos de 3 dígitos após ponto pode ser decimal americano: "10.5" = 10.5
          // Mas como estamos no contexto brasileiro, vamos tratar como milhares também
          // para evitar confusão - usuário deve usar vírgula para decimais
          const onlyDigits = cleanValue.replace(/\./g, '');
          const result = parseInt(onlyDigits);
          return isNaN(result) ? 0 : result;
        } else {
          // Mais de 6 dígitos após ponto - formato inválido, remover pontos
          const onlyDigits = cleanValue.replace(/\./g, '');
          const result = parseInt(onlyDigits);
          return isNaN(result) ? 0 : result;
        }
