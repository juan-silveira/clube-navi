...
const useCurrencyMask = (initialValue = '') => {
  ...
  // Função para validar se o valor é válido (mínimo R$ 0,01)
  const isValidAmount = useCallback((val = value) => {
    const numericValue = getNumericValue(val);
    const isValid = numericValue >= 0.01;
    // console.log('🔍 [isValidAmount] Value:', val, 'Numeric:', numericValue, 'Valid:', isValid, '>=0.01?', numericValue >= 0.01);
    return isValid;
