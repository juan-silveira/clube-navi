...
const useTransactions = (initialParams = {}) => {
  ...
  const fetchTransactions = useCallback(async (params = {}) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        // console.log('🔥 [useTransactions] Transações recebidas do backend:', response.data.transactions?.length);
        // console.log('🔥 [useTransactions] Primeira transação:', response.data.transactions?.[0]);
  ...
  const transformedTransactions = transactions.map(tx => {
    // DEBUG: Log cada transação sendo processada
    if (tx.transactionType === 'exchange') {
      // console.log('🔥 [useTransactions] Processando transação EXCHANGE:', tx);
    }
    ...
    } else if (tx.transactionType === 'exchange') {
      type = 'exchange';
      // Para exchange, o functionName indica se é buy, sell ou cancel
      // buy = crédito (recebendo tokens), sell = débito (vendendo tokens)
      if (tx.functionName === 'buy') {
        subType = 'credit';
        amount = parseDecimalValue(tx.amount) || 0;
      } else if (tx.functionName === 'sell') {
        subType = 'debit';
        amount = -(parseDecimalValue(tx.amount) || 0);
      } else if (tx.functionName === 'cancel') {
        subType = 'debit';
        amount = 0; // Cancelamento não afeta saldo
      } else {
        // Fallback genérico
        subType = 'debit';
        amount = parseDecimalValue(tx.amount) || 0;
