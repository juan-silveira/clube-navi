...
const NotificationPage = () => {
  ...
  const deleteNotification = async (notificationId) => {
    ...
    try {
      ...
      showSuccess(t('alerts.movedToTrash'), t('alerts.deletionCompleted'));
    } catch (error) {
  ...
  const restoreNotification = async (notificationId) => {
    ...
    try {
      ...
      showSuccess(t('alerts.notificationRestored'), t('alerts.restorationCompleted'));
    } catch (error) {
      console.error('Erro ao restaurar notificação:', error);
      showError(t('alerts.errorRestoringNotification'));
    }
  };
  ...
  const deleteMultipleNotifications = async () => {
    ...
    try {
      ...
      // Mostrar alerta de sucesso
      showSuccess(t('alerts.notificationsMovedToTrash'), t('alerts.deletionCompleted'));
    } catch (error) {
      console.error('Erro ao excluir múltiplas notificações:', error);
      showError(t('alerts.errorDeletingNotifications'));
    }
  };
  ...
  const deleteAllNotifications = async () => {
    try {
      ...
      // Mostrar alerta de sucesso
      showSuccess(t('alerts.allNotificationsMovedToTrash'), t('alerts.deletionCompleted'));
    } catch (error) {
  ...
  const restoreMultipleNotifications = async (notificationIds) => {
    try {
      ...
      showSuccess(t('alerts.notificationsRestored'), t('alerts.restorationCompleted'));
    } catch (error) {
      console.error('Erro ao restaurar notificações:', error);
      showError(t('alerts.errorRestoringNotifications'));
    }
  };
  ...
  const restoreAllNotifications = async (notificationIds) => {
    try {
      ...
      showSuccess(t('alerts.allNotificationsRestored'), t('alerts.restorationCompleted'));
    } catch (error) {
      console.error('Erro ao restaurar notificações:', error);
      showError(t('alerts.errorRestoringNotifications'));
    }
  ...
  return (
    <>
      <div className="min-h-screen bg-gray-50 dark:bg-slate-900 py-8">
        <div className="px-2 sm:px-4 lg:px-6">
            ...
            <div className="mb-6 bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-gray-200 dark:border-slate-700 p-4">
              ...
              <div className="grid grid-cols-2 gap-3 lg:flex lg:items-center lg:justify-between lg:space-x-4 lg:gap-0">
                ...
                <div className="col-span-1 lg:col-span-1 flex items-center space-x-3">
                  <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{t('filter.label')}</span>
                  <select
                    value={filter}
                    onChange={(e) => {
                  ...
                  >
                    ...
                    <option value="favorite">{t('filter.favorite')}</option>
                    <option value="deleted">{t('filter.deleted')}</option>
                  </select>
                </div>
                ...
                <div className="col-span-1 text-sm text-gray-600 dark:text-gray-400 flex items-center justify-end">
                  {notifications.length === 1 ? t('counter.notifications_one', { count: notifications.length }) : t('counter.notifications_other', { count: notifications.length })}
                </div>
                ...
                {selectedNotifications.size > 0 && (
                  <div className="col-span-1 text-sm text-gray-700 dark:text-gray-300 flex items-center justify-end">
                    {t('counter.selected', { count: selectedNotifications.size })}
                  </div>
                )}
          ...
          <div className={`bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-gray-200 dark:border-slate-700 overflow-hidden relative w-full ${
            selectedNotification ? 'min-h-[600px]' : 'min-h-[60px]'
          }`}>
            {loading ? (
              ...
              <div className="text-center py-12">
                ...
                <p className="text-gray-600 dark:text-gray-400">
                  {filter === 'all' ? t('emptyState.all') :
                   filter === 'unread' ? t('emptyState.unread') :
                   filter === 'read' ? t('emptyState.read') :
                   filter === 'favorite' ? t('emptyState.favorite') :
                   filter === 'deleted' ? t('emptyState.deleted') :
                            ...
                            <button
                              ...
                              className={`px-2 py-1 text-xs font-medium rounded ${
                                typeof page === 'number' && currentPage === page
                                  ...
                                  : page === '...'
                                  ? 'text-gray-400 cursor-default dark:text-slate-500'
      ...
      <ConfirmationModal
        ...
        title={t('modals.deleteMultiple.title')}
        message={t('modals.deleteMultiple.message', { count: selectedNotifications.size })}
        confirmText={t('modals.deleteMultiple.confirm')}
        cancelText={t('modals.deleteMultiple.cancel')}
      ...
      <ConfirmationModal
        ...
        title={t('modals.deleteAll.title')}
        message={t('modals.deleteAll.message', { count: filteredNotifications.length })}
        confirmText={t('modals.deleteAll.confirm')}
        cancelText={t('modals.deleteAll.cancel')}
        confirmButtonColor="red"
        icon="danger"
      />
