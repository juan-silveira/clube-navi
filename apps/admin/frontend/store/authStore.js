...
const useAuthStore = create(
  persist(
    (set, get) => ({
      ...
      currentCompanyAlias: null, // alias da empresa atual do usuário
      ...
      setCurrentCompanyAlias: (alias) => set({ currentCompanyAlias: alias }),
      ...
      login: (user, accessToken, refreshToken, requiresPasswordChange = false) => {
        ...
        set({
          ...
          profilePhotoUrl: photoUrl, // Usar foto do perfil do banco se existir
          profilePhotoTimestamp: photoUrl ? Date.now() : null,
        });
      },
      ...
      logout: (reason = 'manual') => {
        ...
        // PRIORIDADE 0: Usar currentCompanyAlias armazenado no store (mais confiável)
        if (state.currentCompanyAlias) {
          companyAlias = state.currentCompanyAlias;
        }

        // PRIORIDADE 1: Obter do sessionStorage (salvo no login)
        ...
        // PRIORIDADE 2: Obter do CompanyContext (company da sessão atual)
        if (companyAlias === 'coinage') {
          try {
        ...
        // PRIORIDADE 3: Se não encontrou no CompanyContext, tentar do usuário
        if (companyAlias === 'coinage' && state.user?.company_alias) {
          companyAlias = state.user.company_alias;
        }

        // PRIORIDADE 4: Se ainda não encontrou, tentar obter da URL atual
        if (companyAlias === 'coinage') {
          try {
            const currentPath = window.location.pathname;
            const pathMatch = currentPath.match(/\/(login|register)\/([^\/\?]+)/);
        ...
        set({
          ...
          balancesLastUpdate: null,
          balancesLoading: false,
          currentCompanyAlias: null, // limpar company alias ao fazer logout
        });
      ...
      setCachedBalances: (balances) => {
        ...
        if (!hasValidBalances && (!state.cachedBalances || Object.keys(state.cachedBalances.balancesTable || {}).length === 0)) {
          ...
          // Obter rede da configuração ou usar mainnet como padrão
          let defaultNetwork = 'mainnet';
          try {
            const configData = localStorage.getItem('app:public-config');
            if (configData) {
              const config = JSON.parse(configData);
              defaultNetwork = config.defaultNetwork || 'mainnet';
            }
          } catch (e) {
            // Usar mainnet como padrão
          }
          ...
          const emergencyBalances = {
            ...
            address: state.user?.publicKey || state.user?.address || '',
            lastUpdated: new Date().toISOString(),
      ...
      clearCachedBalances: () => {
        ...
        const nativeToken = defaultNetwork === 'testnet' ? 'AZE-t' : 'AZE';
        const userAddress = get().user?.publicKey || get().user?.address || '';
    ...
    }),
    {
      ...
      partialize: (state) => ({
        ...
        // Persistir foto de perfil para evitar recarregamento
        profilePhotoUrl: state.profilePhotoUrl,
        profilePhotoTimestamp: state.profilePhotoTimestamp,

        // Persistir company alias para redirecionamento correto no logout
