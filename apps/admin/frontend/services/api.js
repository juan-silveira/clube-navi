...
const api = axios.create({
  ...
  timeout: 60000, // 60 segundos para uploads de arquivos grandes
  headers: {
  ...
  async (error) => {
    ...
    if (error.response?.status === 401 && !originalRequest._retry) {
      ...
      if (isAuthenticated && refreshToken) {
        try {
          ...
          if (isTokenInvalid) {
            ...
            const companyAlias = logout('invalid_refresh_token');
            setTimeout(() => window.location.href = `/login/${companyAlias}`, 1000);
          }
      ...
      if (isAuthenticated && !isSyncRequest && !refreshToken) {
        ...
        const companyAlias = logout('no_refresh_token');
        setTimeout(() => window.location.href = `/login/${companyAlias}`, 1000);
      } else {
