...
// Usar token nativo correto baseado na rede
const getEmergencyBalances = (network = 'mainnet') => {
  const nativeToken = network === 'testnet' ? 'AZE-t' : 'AZE';
  return {
...
class BalanceBackupService {
  ...
  getEmergencyBalances(userId) {
    // Obter rede da configuração salva ou usar mainnet como padrão
    let defaultNetwork = 'mainnet';
    
    try {
      // Tentar obter do localStorage a configuração mais recente
      ...
      // console.warn('⚠️ [BalanceBackup] Erro ao obter configuração, usando mainnet');
    }
    ...
    // Obter endereço do usuário do authStore ou localStorage
    let userAddress = '';
    try {
      const authData = localStorage.getItem('auth-storage');
      if (authData) {
        const auth = JSON.parse(authData);
        userAddress = auth?.state?.user?.publicKey || auth?.state?.user?.address || '';
      }
    } catch (e) {
      // Falha silenciosa
    }
    ...
    return {
      data: {
        balancesTable: { ...getEmergencyBalances(defaultNetwork) },
