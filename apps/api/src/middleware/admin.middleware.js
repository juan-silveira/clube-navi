...
/**
 * Middleware para verificar se o usuário é APP_ADMIN ou SUPER_ADMIN
 */
const requireSuperAdmin = async (req, res, next) => {
  try {
    ...
    // Verificar se o usuário tem role APP_ADMIN ou SUPER_ADMIN
    const userCompanies = req.user.userCompanies || [];
    const isSuperAdmin = userCompanies.some(uc =>
    ...
    if (!isSuperAdmin) {
      return res.status(403).json({
        success: false,
        message: 'Acesso negado: apenas APP_ADMIN ou SUPER_ADMIN podem acessar',
