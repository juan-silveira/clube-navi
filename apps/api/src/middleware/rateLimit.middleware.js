...
const createRateLimiter = (options) => {
  ...
  return (req, res, next) => {
    ...
    // Se não conseguir identificar a empresa, usar IP como fallback
    const rateLimitKey = companyKey ? 
      `${keyPrefix}:${companyKey}` : 
      `${keyPrefix}:ip:${req.ip || req.connection.remoteAddress || 'unknown'}`;
...
// Limpar cache na inicialização para evitar problemas com 429
clearRateLimit();
