 ...
 */
contract StakingContractName is AccessControl, ReentrancyGuard {
    ...
    // VARIÁVEIS DE ESTADO
    PratiqueCoin public stakeToken;
    PratiqueCoin public rewardToken;
    ...
    event CycleStartTimeUpdated(uint256 newStartTime); // <<< ADICIONADO
    ...
    constructor(
        ...
        stakeToken = PratiqueCoin(stakeToken_);
        rewardToken = PratiqueCoin(rewardToken_);
    ...
    /**
     * @dev Atualiza o timestamp de início do ciclo de recompensas atual.    // <<< ADICIONADO
     * @notice Apenas o administrador pode chamar esta função.
     * @param _newStartTime O novo timestamp (Unix time) para o início do ciclo.
     */
    function updateInitialCycleStartTime(uint256 _newStartTime) external onlyRole(DEFAULT_ADMIN_ROLE) { // <<< ADICIONADO
        require(_newStartTime > 0, "New start time must be positive.");
        require(_newStartTime <= block.timestamp, "Start time cannot be in the future.");
    ...
    /**
     * @dev Retorna o timestamp de início do ciclo de recompensas atual.     // <<< ADICIONADO
     * @return O timestamp (Unix time) do início do ciclo.
     */
    function getInitialCycleStartTime() external view returns (uint256) { // <<< ADICIONADO
