...
class MintService {
   ...
   */
  async mintCBRL(recipientAddress, amount, network = null, depositId = null) {
    try {
      ...
      return {
        ...
        explorerUrl: network === 'testnet'
          ? `https://floripa.azorescan.com/tx/${result.transactionHash}`
          : `https://azorescan.com/tx/${result.transactionHash}`,
        depositId
      };
  ...
  async processDepositMint(deposit) {
    try {
      ...
      // Determinar a rede (usar mainnet por padrão em produção)
