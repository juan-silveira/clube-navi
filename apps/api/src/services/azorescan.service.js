...
class AzoreScanService {
  constructor() {
    // Usar APIs das variáveis de ambiente para comunicação com a blockchain
    this.apiUrls = {
      testnet: process.env.TESTNET_EXPLORER_API || 'https://floripa.azorescan.com/api',
      mainnet: process.env.MAINNET_EXPLORER_API || 'https://azorescan.com/api'
    };
    // URLs do explorer para links de visualização
    this.explorerUrls = {
      testnet: process.env.TESTNET_EXPLORER_URL || 'https://floripa.azorescan.com',
      mainnet: process.env.MAINNET_EXPLORER_URL || 'https://azorescan.com'
    };
    // Obter a rede padrão do .env
    this.defaultNetwork = process.env.DEFAULT_NETWORK || 'testnet';
  }

  /**
   * Obter URL da API para a rede especificada
   */
  getApiUrl(network = this.defaultNetwork) {
    return this.apiUrls[network] || this.apiUrls[this.defaultNetwork];
  }

  /**
   * Obter URL do explorer para links de visualização
   */
  getExplorerUrl(network = this.defaultNetwork) {
    return this.explorerUrls[network] || this.explorerUrls[this.defaultNetwork];
  }

  /**
   * Fazer requisição para a API do AzoreScan
   */
  async makeRequest(params, network = this.defaultNetwork) {
    try {
      const apiUrl = this.getApiUrl(network);
      // console.log(`🌐 Fazendo requisição para AzoreScan API ${network}: ${apiUrl}`);
   ...
   */
  async getTokenList(address, network = this.defaultNetwork) {
   ...
   */
  async getTokenBalance(contractAddress, address, network = this.defaultNetwork) {
   ...
   */
  async getCompleteBalances(address, network = this.defaultNetwork) {
    try {
