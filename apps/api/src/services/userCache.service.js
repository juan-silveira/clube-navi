  ...
  async clearUserCache(userId) {
    try {
      ...
      const cacheKeys = [
        ...
        // REMOVIDO: profile:photo cache desabilitado por segurança
      ];
      ...
      // Limpar também cache de balanços de token usando padrão de chaves
      try {
        const tokenCacheKeys = await redisService.company.keys(`previous_balances:${userId}:*`);
        if (tokenCacheKeys.length > 0) {
          await redisService.company.del(tokenCacheKeys);
          console.log(`🗑️ Limpos ${tokenCacheKeys.length} caches de token para usuário: ${userId}`);
        }
      } catch (tokenError) {
        console.warn(`⚠️ Erro ao limpar cache de tokens para usuário ${userId}:`, tokenError.message);
      }
      
      console.log(`🗑️ Cache completo limpo para usuário: ${userId}`);
      return true;
    } catch (error) {
