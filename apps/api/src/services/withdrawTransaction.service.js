...
class WithdrawTransactionService {
  ...
  /**
   * Atualizar transação com confirmação PIX
   */
  async updateWithPixConfirmation(transactionId, pixConfirmationData) {
    try {
      if (!this.prisma) await this.init();

      const { pixEndToEndId, confirmedAt, confirmedAmount, status } = pixConfirmationData;
        ...
        data: {
          ...
          metadata: {
            ...
            value: {
              ...
              confirmedAt: confirmedAt ? confirmedAt.toISOString() : new Date().toISOString(),
      ...
      console.log(`✅ Transação atualizada com confirmação PIX: ${transactionId} - E2E: ${pixEndToEndId}`);
      return updated;

    } catch (error) {
      console.error('❌ Erro ao atualizar transação com confirmação PIX:', error);
      throw error;
    }
  }
}
