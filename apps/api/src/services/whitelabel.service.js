...
class WhitelabelService {
  ...
  /**
   * Atualiza branding da empresa
   * @param {string} companyId - ID da empresa
   * @param {Object} brandingData - Dados do branding para atualizar
   * @returns {Promise<Object>} Branding atualizado
   */
  async updateCompanyBranding(companyId, brandingData) {
    try {
      ...
      const existingBranding = await this.prisma.companyBranding.findUnique({
        where: { companyId }
      });
      ...
      if (existingBranding) {
        ...
        updatedBranding = await this.prisma.companyBranding.update({
          where: { companyId },
          data: {
            ...brandingData,
            updatedAt: new Date()
        ...
        // Criar novo branding
        updatedBranding = await this.prisma.companyBranding.create({
          data: {
            companyId,
            ...brandingData
          }
      ...
      console.error('❌ [WhitelabelService] Erro ao atualizar branding:', error);
      throw error;
    }
  }
  ...
  async getCompanyBrandingByAlias(companyAlias) {
    try {
      ...
      const branding = company.companyBrandings; // Relação 1:1 opcional
      console.log('🔍 Debug branding getCompanyBrandingByAlias:', { branding, isActive: branding?.isActive });

      if (!branding || !branding.isActive) {
        console.log('🔍 Branding não encontrado ou inativo - retornando null');
        // Retornar null para que o frontend use o fallback de emergência
        return null;
      }
      ...
      return {
        ...
        mini_url: branding.miniUrl,
        mini_url_dark: branding.miniUrlDark,
        text_url: branding.textUrl,
        text_url_dark: branding.textUrlDark,
        background_image_url: branding.backgroundImageUrl || null,
        custom_css: branding.customCss || null,
    ...
    try {
      ...
      return {
        ...
        branding: {
          ...
          miniUrl: branding.miniUrl,
          miniUrlDark: branding.miniUrlDark,
          textUrl: branding.textUrl,
          textUrlDark: branding.textUrlDark,
  ...
  /**
   * Lista empresas disponíveis com seus aliases
   */
  async getAvailableCompanies() {
    try {
      ...
      // Retornar empresas com dados do branding se disponível
      return companies.map(company => ({
        id: company.id,
        name: company.name,
        alias: company.alias,
        brandName: company.companyBrandings[0]?.loginTitle || company.name,
        logoUrl: company.companyBrandings[0]?.logoUrl || null,
        miniUrl: company.companyBrandings[0]?.miniUrl || null
      }));
    } catch (error) {
