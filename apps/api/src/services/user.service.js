...
class UserService {
  ...
  async getUserByPublicKey(publicKey, includePrivateKey = false) {
    try {
      ...
      console.log(`🔍 [UserService] Buscando usuário com publicKey: ${publicKey}`);
      ...
      // Se não encontrou, tentar busca exata (fallback)
      if (!user) {
        console.log(`⚠️ Tentativa case-insensitive falhou, tentando busca exata...`);
      ...
      if (!user) {
        console.log(`❌ Usuário não encontrado para publicKey: ${publicKey}`);

        // Debug: buscar todos os usuários para ver os publicKeys
        const allUsers = await this.prisma.user.findMany({
          where: { isActive: true },
          select: { id: true, name: true, publicKey: true }
        });
        console.log(`📋 Total de usuários ativos: ${allUsers.length}`);
        if (allUsers.length > 0 && allUsers.length < 10) {
          console.log(`📋 PublicKeys cadastrados:`, allUsers.map(u => u.publicKey));
        }
  ...
  async listUsers(options = {}) {
    try {
      ...
      // Buscar dados de atividade (login count, transações, último login) para cada usuário
      const usersWithActivity = await Promise.all(users.map(async (user) => {
        try {
          // Buscar count de logins da tabela user_actions
          const loginCount = await this.prisma.UserAction.count({
            where: {
              userId: user.id,
              action: 'login'
            }
          ...
          // Buscar último login da tabela user_actions
          const lastLoginAction = await this.prisma.UserAction.findFirst({
            where: {
              userId: user.id,
              action: 'login'
            },
          ...
          // Buscar count de transações da tabela transactions
          ...
          // Adicionar dados de atividade ao usuário
          return {
            ...user,
            loginCount,
            transactionCount,
            lastActivityAt: lastLoginAction?.performedAt || null,
            avatar: user.profilePicture || user.profile_picture || null
          };
        } catch (error) {
          console.error(`Erro ao buscar dados de atividade para usuário ${user.id}:`, error);
          // Se der erro, retornar com dados zerados
          return {
            ...user,
            loginCount: 0,
            transactionCount: 0,
      ...
      // Sanitizar usuários
      const sanitizedUsers = usersWithActivity.map(user => this.sanitizeUser(user));
