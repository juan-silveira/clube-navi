...
class ContractService {
  ...
  async writeContract(contractAddress, functionName, params = [], walletAddress, options = {}) {
    try {
      ...
      // Para funções administrativas simples como updateInitialCycleStartTime, usar timeout menor
      if (functionName === 'updateInitialCycleStartTime') {
        // Aguardar apenas 1 confirmação com timeout de 60s para funções admin simples
        const timeoutPromise = new Promise((_, reject) => {
          setTimeout(() => reject(new Error('Timeout: Função administrativa levou mais que 60s para confirmar')), 60000);
        });

        try {
          receipt = await Promise.race([
            tx.wait(1), // 1 confirmação apenas
            timeoutPromise
          ]);
        } catch (error) {
          if (error.message.includes('Timeout')) {
            // Se deu timeout, mas a transação pode ter sido enviada, retornar parcialmente
            return {
              success: true,
              message: 'Transação enviada com sucesso. Confirmação pode levar alguns minutos.',
              data: {
                contractAddress: contract.address,
                functionName,
                params,
        ...
        // Para outras funções, usar comportamento padrão
