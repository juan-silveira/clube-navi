...
class WithdrawService {
  ...
  async initiateWithdrawal(amount, pixKey, userId) {
    try {
      ...
      console.log(`🔗 [WithdrawService] Consultando saldo cBRL na blockchain para: ${user.publicKey} (${network})`);
      const blockchainBalances = await blockchainService.getUserBalances(user.publicKey, network);
      ...
      const withdrawal = await this.prisma.withdrawal.create({
        data: {
          ...
          pixKeyType: userPixKey ? userPixKey.keyType : this.detectPixKeyType(pixKey),
          status: 'PENDING',
          createdAt: new Date(),
          metadata: {
            cBrlBalance: cBrlBalance,
  ...
  async confirmWithdrawal(withdrawalId) {
    ...
    try {
      ...
      // Marcar PIX como pendente (aguardando processamento manual)
  ...
  /**
   * Marcar pagamento PIX como pendente (processamento manual)
   */
  async processPixPayment(withdrawalId, withdrawal) {
    try {
      console.log(`💳 Marcando pagamento PIX como pendente para saque ${withdrawalId}`);

      // Atualizar withdrawal para status PENDING com pix_status PENDING
      const updatedWithdrawal = await this.prisma.withdrawal.update({
        where: { id: withdrawalId },
        data: {
          status: 'PENDING',
      ...
      // Atualizar transação padronizada para status PENDING
      const standardTransaction = await withdrawTransactionService.findByWithdrawalId(withdrawalId);
      if (standardTransaction) {
        await this.prisma.transaction.update({
          where: { id: standardTransaction.id },
          data: {
            status: 'pending',
            pix_status: 'pending'
          }
        });
        console.log(`✅ Transação padronizada marcada como PENDING`);
      }

      console.log(`✅ Saque ${withdrawalId} marcado como PENDING aguardando processamento manual`);

      return {
        success: true,
        status: 'PENDING',
        message: 'Saque pendente aguardando processamento manual',
        processedAt: new Date().toISOString()
      };

    } catch (error) {
      console.error('❌ Erro ao marcar PIX como pendente:', error);
