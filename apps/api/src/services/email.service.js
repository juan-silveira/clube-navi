...
const MandrillProvider = require('./providers/mandrill.provider');
const MandrillSMTPProvider = require('./providers/mandrill-smtp.provider');
...
class EmailService {
  constructor() {
    ...
    this.providerInstances = {
      ...
      mandrill: new MandrillSMTPProvider(), // Usar SMTP em vez de API
      mandrillApi: new MandrillProvider() // Manter API como backup
    };
  ...
  async sendEmailWithProvider(emailData) {
    try {
      ...
      // Corrigir estrutura de dados do email
      const toEmail = emailData.to?.email || emailData.to || emailData.toEmail;
      const toName = emailData.to?.name || emailData.toName || '';
      ...
      if (result.success) {
        // Registrar sucesso (apenas se prisma estiver inicializado)
        if (this.prisma) {
          await this.logEmail({
            templateId: emailData.templateId,
            toEmail: toEmail,
        ...
        // Registrar falha (apenas se prisma estiver inicializado)
        if (this.prisma) {
          await this.logEmail({
            templateId: emailData.templateId,
            toEmail: toEmail,
  ...
  async simulateEmailSend(emailData) {
    ...
    console.log(`HTML: ${(emailData.html || emailData.htmlContent)?.substring(0, 200)}...`);

    // Registrar log simulado (apenas se prisma estiver inicializado)
    if (this.prisma) {
      await this.logEmail({
        templateId: emailData.templateId,
        toEmail: toEmail,
    ...
    try {
      ...
      await this.prisma.emailLog.create({
        data: {
          ...
          fromEmail: 'system@coinage.trade',
  ...
  async validateEmailConfirmationToken(token, companyAlias) {
    try {
      ...
      if (!emailLog) {
        console.log('⚠️ Token não encontrado no emailLog:', token.substring(0, 16) + '...');
        return {
          valid: false,
      ...
      if (!metadata || !metadata.userId || !metadata.expiresAt) {
        console.error('❌ Metadata inválido no emailLog:', emailLog.id);
        return {
          valid: false,
          error: 'INVALID_METADATA',
          email: emailLog.toEmail // Retornar email mesmo em caso de erro
        };
      }
      ...
      if (expiresAt < new Date()) {
        console.log('⏰ Token expirado:', token.substring(0, 16) + '...', 'Expirou em:', expiresAt);
        return {
          valid: false,
          error: 'TOKEN_EXPIRED',
          email: emailLog.toEmail, // Retornar email para facilitar reenvio
      ...
      console.log('✅ Token válido para userId:', metadata.userId);
      return {
        userId: metadata.userId,
