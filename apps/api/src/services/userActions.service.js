...
class UserActionsService {
  ...
  async getUserActions(userId, filters = {}) {
    try {
      ...
      console.log('🔍 [UserActionsService] getUserActions para userId:', userId);
      console.log('🔍 [UserActionsService] Filtros recebidos:', filters);
      ...
      console.log('🔍 [UserActionsService] Where clause:', where);

      const [actions, total] = await Promise.all([
        getPrisma().userAction.findMany({
          where,
          take: limit,
          skip: offset,
          orderBy: {
        ...
        getPrisma().userAction.count({ where })
      ]);

      console.log('🔍 [UserActionsService] Actions encontradas:', actions.length);
      console.log('🔍 [UserActionsService] Total:', total);
      console.log('🔍 [UserActionsService] Primeira ação:', actions[0]);
  ...
  async getUserTimeline(userId, limit = 20) {
    try {
      const actions = await getPrisma().userAction.findMany({
        where: {
          userId,
  ...
  async getUserActionStats(userId, period = '30d') {
    try {
      ...
      const stats = await getPrisma().userAction.groupBy({
        by: ['category', 'status'],
        where: {
          userId,
          performedAt: {
  ...
  async detectSuspiciousActivity(userId, action, req) {
    try {
      const recentActions = await getPrisma().userAction.findMany({
        where: {
          userId,
          action,
          performedAt: {
  ...
  async getSuspiciousActivity(filters) {
    try {
      ...
      const suspiciousActions = await getPrisma().userAction.findMany({
        where: {
          OR: [
