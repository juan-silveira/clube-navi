...
class TransactionController {
  ...
  async getTransactionsByCompany(req, res) {
    try {
      ...
      // 🔥 DEBUG: Log primeiras 3 transações recebidas do service
      console.log('🔥 [TransactionController] ORDEM RECEBIDA DO SERVICE (primeiras 3):');
      result.rows?.slice(0, 3).forEach((tx, idx) => {
  ...
  /**
   * Obtém transações de todos os usuários de uma empresa (para admins)
   */
  async getCompanyTransactions(req, res) {
    try {
      const { page = 1, limit = 50, status, network, transactionType, tokenSymbol, startDate, endDate, userId: filterUserId } = req.query;
      const companyId = req.user.companyId;

      console.log('🔍 [TransactionController] Buscar transações da empresa:', {
        companyId,
        filters: { page, limit, status, network, transactionType, tokenSymbol, startDate, endDate, filterUserId }
      });

      if (!companyId) {
        console.error('❌ [TransactionController] CompanyID não encontrado');
        return res.status(400).json({
          success: false,
      ...
      // Registrar visualização de transações da empresa
      await userActionsService.logAction({
        userId: req.user.id,
        companyId: companyId,
        action: 'company_transactions_viewed',
        category: 'admin',
        details: {
          filters: { status, network, transactionType, tokenSymbol, startDate, endDate, filterUserId },
          pagination: { page: parseInt(page), limit: parseInt(limit) }
        },
        ipAddress: userActionsService.getIpAddress(req),
        userAgent: req.headers['user-agent']
      ...
      console.log('🔍 [TransactionController] Chamando transactionService.getTransactionsByCompany...');
      
      // Buscar transações reais da tabela transactions do banco de dados
      const result = await transactionService.getTransactionsByCompany(companyId, {
        page: parseInt(page),
        limit: parseInt(limit),
        status,
        network,
      ...
      const responseData = {
        success: true,
        message: 'Transações da empresa obtidas com sucesso',
        data: {
          transactions: result.rows.map(tx => tx.getFormattedResponse()),
          pagination: {
            total: result.count,
      ...
      console.error('❌ [TransactionController] Erro ao buscar transações da empresa:', {
        error: error.message,
        stack: error.stack,
        companyId: req.user?.companyId
      });
  ...
  /**
   * Obtém tokens disponíveis cadastrados no banco de dados
   */
  async getAvailableTokens(req, res) {
    try {
      const companyId = req.user?.companyId;
      
      console.log('🪙 [TransactionController] Obtendo tokens disponíveis:', { companyId });

      const tokens = await transactionService.getAvailableTokens(companyId);
      ...
      console.error('❌ [TransactionController] Erro ao obter tokens disponíveis:', {
        error: error.message,
        stack: error.stack,
        companyId: req.user?.companyId
      });
      res.status(500).json({
        success: false,
        message: 'Erro ao obter tokens disponíveis',
        error: error.message
      });
    }
  }
