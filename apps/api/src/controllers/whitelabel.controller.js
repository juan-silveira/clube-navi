...
const authenticateUser = async (req, res) => {
  try {
    ...
    console.log('🔐 [WHITELABEL LOGIN] Verificando 2FA para usuário:', user.id, user.email);
    const twoFactorService = require('../services/twoFactor.service');
    const has2FA = await twoFactorService.userHasTwoFactor(user.id);
    console.log('🔐 [WHITELABEL LOGIN] Usuário tem 2FA?', has2FA);

    if (has2FA) {
      const { twoFactorCode } = req.body;
      console.log('🔐 [WHITELABEL LOGIN] Código 2FA fornecido?', !!twoFactorCode);

      // Se tem 2FA mas não forneceu código, retornar que precisa do código
      if (!twoFactorCode) {
        console.log('🔐 [WHITELABEL LOGIN] Solicitando código 2FA ao frontend...');
      ...
      try {
        console.log('🔐 [WHITELABEL LOGIN] Verificando código 2FA...');
        const verificationResult = await twoFactorService.verify2FA(user.id, twoFactorCode);
        console.log('🔐 [WHITELABEL LOGIN] Código 2FA válido?', verificationResult.success, 'método:', verificationResult.method);
        ...
        if (!verificationResult.success) {
          ...
          console.log('❌ [WHITELABEL LOGIN] Código 2FA inválido');
        ...
        console.log('✅ [WHITELABEL LOGIN] Código 2FA válido! Prosseguindo com login...');
        ...
        console.error('❌ [WHITELABEL LOGIN] Erro ao verificar 2FA:', error);
        return res.status(500).json({
          success: false,
...
const getCompanyBrandingByAlias = async (req, res) => {
  try {
    ...
    // Se branding é null (não encontrado/inativo), retornar erro 404
    if (!branding) {
      return res.status(404).json({
        success: false,
        message: 'Branding não encontrado para esta empresa'
      });
    }
...
/**
 * Atualiza configuração de branding da empresa
 */
const updateCompanyBranding = async (req, res) => {
  try {
    const { companyId } = req.params;
    ...
    if (!hasPermission) {
      return res.status(403).json({
        success: false,
        message: 'Sem permissão para atualizar branding desta empresa'
      });
    }

    // Atualizar branding
    const updatedBranding = await whitelabelService.updateCompanyBranding(companyId, brandingData);
