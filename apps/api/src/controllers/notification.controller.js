...
class NotificationController {
  ...
  async sendNotification(req, res) {
    try {
      const { userId, title, message, type, priority, category, metadata } = req.body;
      const sender = req.user.id; // ID do usuário que está enviando
      const companyName = req.company?.name || req.user.currentCompany?.name || 'Sistema'; // Nome da empresa
  ...
  async sendBulkNotification(req, res) {
    try {
      const { userIds, title, message, type, priority, category, metadata } = req.body;
      const sender = req.user.id; // ID do usuário que está enviando
      const companyName = req.company?.name || req.user.currentCompany?.name || 'Sistema'; // Nome da empresa
      ...
      if (!userIds || !Array.isArray(userIds) || userIds.length === 0) {
        return res.status(400).json({
          success: false,
          message: 'Lista de userIds é obrigatória e deve conter pelo menos um usuário'
        });
      }
      ...
      res.status(201).json({
        ...
        message: `${notifications.length} notificação(ões) enviada(s) com sucesso`
      });
    } catch (error) {
