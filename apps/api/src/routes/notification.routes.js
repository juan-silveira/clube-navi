...
const { notificationRateLimit } = require('../middleware/rateLimiter');
...
// Rotas para envio de notificações (requerem autenticação)
router.post('/send', authenticateToken, notificationController.sendNotification.bind(notificationController));
router.post('/send-bulk', authenticateToken, notificationController.sendBulkNotification.bind(notificationController));

// Rotas protegidas (requerem autenticação JWT + rate limiting)
router.get('/', authenticateToken, notificationRateLimit, notificationController.getNotifications.bind(notificationController));
router.get('/unread', authenticateToken, notificationRateLimit, notificationController.getUnreadNotifications.bind(notificationController));
router.get('/unread-count', authenticateToken, notificationRateLimit, notificationController.getUnreadCount.bind(notificationController));
