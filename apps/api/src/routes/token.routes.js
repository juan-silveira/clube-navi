...
// Alternative route pattern that the frontend expects
router.get('/:contractAddress/balance/:walletAddress', async (req, res) => {
  try {
    const { contractAddress, walletAddress } = req.params;
    const { network = 'testnet' } = req.query;

    // Call the existing controller with the right format
    req.query = { contractAddress, walletAddress, network };
    await tokenController.getTokenBalance(req, res);
  } catch (error) {
