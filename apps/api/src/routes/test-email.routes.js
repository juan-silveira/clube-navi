...
router.post('/send', async (req, res) => {
  try {
        ...
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
          ...
          <div style="background: #ecfdf5; padding: 15px; border-radius: 8px; border-left: 4px solid #10b981;">
            <p style="margin: 0; color: #065f46;">
              ✅ <strong>Sistema de Email ativo!</strong><br>
    ...
    res.json({
      ...
      result,
      provider: emailService.activeProvider || 'mock',
...
router.get('/config', async (req, res) => {
  try {
    ...
    const config = {
      ...
      configuredProvider: process.env.EMAIL_PROVIDER,
      providers: {
        mandrill: {
          enabled: !!process.env.MANDRILL_API_KEY,
          fromEmail: process.env.MANDRILL_FROM_EMAIL,
          fromName: process.env.MANDRILL_FROM_NAME,
          hasApiKey: !!process.env.MANDRILL_API_KEY
        },
        mailersend: {
          enabled: !!process.env.MAILERSEND_API_TOKEN,
...
/**
 * @swagger
 * /api/test/email/validate:
 *   get:
 *     tags:
 *       - Test Email
 *     summary: Validar configuração dos provedores de email
 *     responses:
 *       200:
...
router.get('/validate', async (req, res) => {
  try {
    ...
    const validation = await emailService.validateProviders();
    ...
    res.json({
      ...
      instructions: {
        mandrill: {
          setup: 'Para configurar o Mandrill:',
          steps: [
            '1. Acesse https://mandrillapp.com',
            '2. Faça login com sua conta Mailchimp',
            '3. Vá em Settings > API Keys',
            '4. Crie uma nova API Key',
            '5. Adicione a key no arquivo .env como MANDRILL_API_KEY',
            '6. Configure MANDRILL_FROM_EMAIL com seu email verificado'
          ]
        }
      }
    });
    ...
    console.error('❌ Erro ao validar provedores:', error);
    res.status(500).json({
      success: false,
      message: 'Erro ao validar provedores',
      error: error.message
    });
  }
});
