...
// Rate limiter específico para foto de perfil (mais permissivo para evitar bloqueios)
const profilePhotoRateLimiter = createRateLimiter({
  maxRequests: 20, // Máximo 20 requisições (aumentado)
  windowMs: 60 * 1000, // Em 60 segundos (janela maior)
  keyPrefix: 'profile_photo_rate_limit',
  keyGenerator: (req) => req.user?.id || req.ip,
  message: 'Muitas requisições para foto de perfil. Aguarde alguns segundos.',
  skipSuccessfulRequests: true // Não conta requisições bem-sucedidas com cache
});
