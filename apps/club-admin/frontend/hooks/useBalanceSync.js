...
// Função para obter o intervalo baseado no plano do usuário (DESABILITADO - apenas manual)
// const getSyncIntervalMs = (userPlan = 'BASIC') => {
//   switch (userPlan) {
//     case 'PREMIUM': return 5 * 60 * 1000; // 5 minutos para usuários premium  
//     case 'PRO': return 10 * 60 * 1000;    // 10 minutos para usuários pro
//     case 'BASIC':
//     default: return 15 * 60 * 1000;       // 15 minutos para usuários básicos
//   }
// };

// POLLING AUTOMÁTICO TOTALMENTE DESABILITADO - apenas sincronização manual sob demanda
...
const useBalanceSync = (onBalanceUpdate = null) => {
    ...
    try {
      ...
      Object.keys(newTable).forEach(token => {
        if (!(token in prevTable) && parseFloat(newTable[token] || 0) > 0) {
          ...
          const emergencyTokens = ['AZE', 'cBRL'];
  ...
  // PROTEÇÃO CRÍTICA GLOBAL CONTRA MÚLTIPLAS CALLS SIMULTÂNEAS - SISTEMA 5 CAMADAS
  const requestCacheRef = useRef(new Map());
  const activeRequestRef = useRef(null); // SINGLETON: apenas 1 request por vez
  const requestLockRef = useRef(false); // LOCK LOCAL para prevenir race conditions
  const globalLockRef = useRef(false); // LOCK GLOBAL adicional
  const requestCounterRef = useRef(0); // CONTADOR para debug
  const CACHE_TTL_CRITICAL = 8000; // 8 segundos cache para permitir detecção mais rápida
  
  // PROTEÇÃO GLOBAL CROSS-INSTANCE
  if (typeof window !== 'undefined' && !window.__balanceSyncGlobalState) {
    window.__balanceSyncGlobalState = {
  ...
  const getBalancesWithFallback = useCallback(async () => {
    // 🚨 SEMPRE RETORNAR QUE PRECISA BUSCAR DA BLOCKCHAIN
    return {
      data: null,
      source: 'blockchain_only',
      success: false,
      needsFetch: true,
      error: 'Sempre buscar da blockchain'
    };
      return {
        data: cached.data,
        source: 'protected_cache',
        success: true,
        fromCache: true
      };
    }

    // LOCK GLOBAL CRÍTICO: Prevenir múltiplas instâncias do hook
    if (typeof window !== 'undefined') {
      if (window.__balanceSyncLock || window.__balanceSyncGlobalState?.isRequestActive) {
        // console.log('🚫 [BalanceSync] PROTEÇÃO GLOBAL: Request bloqueado por lock global ou estado global ativo');
        return {
          data: null,
          source: 'blocked_by_global_lock',
          success: false,
          error: 'Request bloqueado globalmente para prevenir spam'
        };
      }
    }

    // LOCK CRÍTICO MÚLTIPLO: Prevenir race conditions
    if (requestLockRef.current || globalLockRef.current) {
      // console.log('🚫 [BalanceSync] PROTEÇÃO LOCAL: Request bloqueado por locks locais');
      return {
        data: null,
        source: 'blocked_by_lock',
        success: false,
        error: 'Request bloqueado para prevenir spam'
      };
    }

    // SINGLETON CRÍTICO: Se já há request ativo, aguardar o resultado
    if (activeRequestRef.current) {
      // console.log('🛡️ [BalanceSync] PROTEÇÃO 429: Aguardando request ativo...');
      try {
        return await activeRequestRef.current;
      } catch (error) {
        console.warn('⚠️ [BalanceSync] Request ativo falhou, continuando...');
      }
    }
    
    // INCREMENTAR CONTADOR GLOBAL PARA DEBUG
    requestCounterRef.current++;
    const requestId = requestCounterRef.current;
    
    // PROTEÇÃO ADICIONAL: Se já existe promise global ativa, aguardar ela
    if (typeof window !== 'undefined' && window.__balanceSyncGlobalState?.activePromise) {
      // console.log(`🛡️ [BalanceSync] PROTEÇÃO GLOBAL: Request ${requestId} aguardando promise global ativa...`);
      try {
        return await window.__balanceSyncGlobalState.activePromise;
      } catch (error) {
        console.warn(`⚠️ [BalanceSync] Promise global falhou para request ${requestId}, continuando...`);
      }
    }
    
    // ATIVAR LOCKS CRÍTICOS EM TODAS AS CAMADAS
    requestLockRef.current = true;
    globalLockRef.current = true;
    if (typeof window !== 'undefined') {
      window.__balanceSyncLock = true;
      window.__balanceSyncGlobalState.isRequestActive = true;
    }
    
    // console.log(`🚀 [BalanceSync] SINGLETON START: Request ${requestId} iniciando...`);

    // CRIAR PROMISE SINGLETON PARA EVITAR MÚLTIPLAS CALLS
    const requestPromise = (async () => {
      try {
        ...
        try {
          ...
          // console.log(`🚀 [BalanceSync] Request ${requestId} fazendo API call para fresh balance...`);
      ...
      if (response.data.success && response.data.data && response.data.data.balancesTable && Object.keys(response.data.data.balancesTable).length > 0) {
        ...
        // console.log(`✅ [BalanceSync] Request ${requestId} API success, salvando backups...`);
        
        // SALVAR EM TODOS OS BACKUPS
        await balanceBackupService.saveBalances(user.id, response.data.data, 'api');
        
        // SALVAR NO CACHE CRÍTICO PARA PREVENIR 429
    ...
    if (backupResult && backupResult.data) {
      ...
      return {
        ...
        console.error('🚨 [BalanceSync] Erro na função principal:', globalError);
        return {
          data: null,
          source: 'global_error',
          success: false,
    ...
    // REGISTRAR REQUEST ATIVO EM TODAS AS CAMADAS
    activeRequestRef.current = requestPromise;
    if (typeof window !== 'undefined') {
      window.__balanceSyncGlobalState.activePromise = requestPromise;
    }
    
    try {
      const result = await requestPromise;
      // console.log(`✅ [BalanceSync] SINGLETON SUCCESS: Request ${requestId} concluído, source: ${result.source}`);
      return result;
    } catch (error) {
      console.error(`❌ [BalanceSync] SINGLETON ERROR: Request ${requestId} falhou:`, error.message);
      throw error;
    } finally {
      // LIMPAR REQUEST ATIVO E TODOS OS LOCKS
      // console.log(`🧹 [BalanceSync] SINGLETON CLEANUP: Request ${requestId} limpando locks...`);
      activeRequestRef.current = null;
      requestLockRef.current = false;
      globalLockRef.current = false;
      if (typeof window !== 'undefined') {
        window.__balanceSyncLock = false;
        if (window.__balanceSyncGlobalState) {
          window.__balanceSyncGlobalState.isRequestActive = false;
          window.__balanceSyncGlobalState.activePromise = null;
        }
      }
    }
  ...
  const syncBalances = useCallback(async (bypassActiveCheck = false) => {
    // 🚨 COMPLETAMENTE DESABILITADO - SEMPRE RETORNAR IMEDIATAMENTE
    console.log('🚨 [BalanceSync] DESABILITADO - syncBalances não executará nenhuma ação');
    return;
      ...
      const previousWasEmergency = previousBalances && (
        ...
        // REMOVER: não bloquear por cache - permite detecção após cache
      );
      
      // FACILITAR DETECÇÃO: ser menos restritivo para permitir notificações
      const shouldDetectChanges = isFromAPI && 
                                 hasPreviousBalances && 
                                 hasValidNewBalances &&
                                 !previousWasEmergency && // Apenas bloquear emergência real
  ...
  const startSync = useCallback(async () => {
    ...
    // HABILITAR POLLING INTELIGENTE PARA DETECTAR MUDANÇAS (com proteção 429)
    const getSyncInterval = (userPlan = 'BASIC') => {
      switch (userPlan) {
        case 'PREMIUM': return 30000;  // 30 segundos para premium
        case 'PRO': return 60000;      // 60 segundos para pro
        case 'BASIC':
        default: return 120000;        // 120 segundos para básico
      }
    };

    // Iniciar polling com intervalo baseado no plano
    const syncInterval = getSyncInterval(user?.userPlan);
    
    syncIntervalRef.current = setInterval(async () => {
      if (!isActiveRef.current || !isAuthenticated || !user?.publicKey) {
        return;
      }
      
      try {
        // console.log(`🔄 [BalanceSync] Polling automático (${syncInterval/1000}s) - plano: ${user?.userPlan || 'BASIC'} - tentando sync...`);
        
        // VERIFICAR SE JÁ HÁ REQUEST ATIVO ANTES DE INICIAR NOVO
        if (activeRequestRef.current || requestLockRef.current || globalLockRef.current) {
          // console.log('🛡️ [BalanceSync] Polling: Request já ativo, pulando...');
          return;
        }
        
        await syncBalances();
      } catch (error) {
        console.warn('⚠️ [BalanceSync] Erro no polling automático:', error.message);
      }
    }, syncInterval);
  ...
  useEffect(() => {
    if (!isAuthenticated || !user?.publicKey || !user?.id) {
      ...
      // LIMPAR TODOS OS CACHES E LOCKS CRÍTICOS 429
