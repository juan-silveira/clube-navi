  ...
  const fetchTransactionStatus = useCallback(async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // Determinar status baseado em pix_status e blockchain_status
        let newStatus = tx.status;

        // Se PIX foi confirmado mas blockchain ainda está pendente, considerar como "processing"
        if (tx.pix_status === 'confirmed' && (!tx.blockchain_status || tx.blockchain_status === 'pending')) {
          newStatus = 'processing_mint';
        } else if (tx.pix_status === 'confirmed' && tx.blockchain_status === 'confirmed') {
          newStatus = 'confirmed';
        }
        ...
        // Callback para confirmação do PIX
        if (tx.pix_status === 'confirmed' && status !== 'pix_confirmed' && onStatusChange) {
          onStatusChange('pix_confirmed', tx);
        }

        // Callback para confirmação completa (PIX + blockchain)
        ...
        // Redirecionar automaticamente SOMENTE quando tudo estiver confirmado
        if (autoRedirect && tx.pix_status === 'confirmed' && tx.blockchain_status === 'confirmed') {
          router.push(`${redirectPath}/${transactionId}`);
        }

        // Continuar polling enquanto não finalizar completamente
        // Parar apenas quando PIX E blockchain estiverem confirmados ou falhados
