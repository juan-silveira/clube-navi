...
class StakeContractsService {
  ...
  async categorizeStakeContracts(userAddress, forceRefresh = false) {
    try {
      ...
      // Process contracts - backend already includes whitelist info, no need for additional calls
      allStakeContracts.forEach(contract => {
        try {
          ...
          // Backend already includes whitelistEnabled and userWhitelisted
          const isWhitelistEnabled = contract.whitelistEnabled === true;
          const isUserWhitelisted = contract.userWhitelisted === true;

          if (isWhitelistEnabled) {
            // Se whitelist está habilitada, verificar se usuário está autorizado
            if (userAddress && isUserWhitelisted) {
              privateOffers.push({
                ...contract,
            ...
            // Usuário não está na whitelist, não incluir
          } else {
