...
const DepositConfirmationLoader = ({ transactionId, onCancel }) => {
  ...
  const isProcessingMint = status === 'processing_mint';
  const isConfirmed = status === 'confirmed';
  const isFailed = status === 'failed';

  // Renderizar confirmação SOMENTE se totalmente finalizada (PIX + blockchain)
      ...
      <Card className="p-6">
        <div className="text-center">
          ...
          <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-2">
            {isProcessingMint ? 'Processando Mint' : 'Aguardando Confirmação'}
          </h2>

          {/* Descrição */}
          <p className="text-gray-600 dark:text-gray-400 mb-6">
            {isProcessingMint
              ? 'PIX confirmado! Agora estamos criando seus tokens cBRL na blockchain...'
              : 'Seu depósito está sendo processado. Isso pode levar alguns minutos.'
            }
          </p>
          ...
          <div className="space-y-2 text-sm text-gray-600 dark:text-gray-400 mb-6">
            <div className="flex justify-between">
              ...
              <span className="font-medium">
                {isProcessingMint ? 'Criando cBRL...' : isPending ? 'Processando' : 'Verificando...'}
              </span>
            </div>
