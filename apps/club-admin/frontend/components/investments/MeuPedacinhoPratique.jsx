...
import StatementsModal from './modals/StatementsModal';
...
const MeuPedacinhoPratique = ({ onContractsLoaded }) => {
  const { t } = useTranslation('investments');
  const { user } = useAuth();
  const { showError } = useAlertContext();
  const { getBalance, balances } = useCachedBalances();
  ...
  // Load contracts com investment_type = 'pratique' e usuário na whitelist
  useEffect(() => {
  ...
  useEffect(() => {
    const loadContractData = async () => {
      ...
      if (pratiqueContracts.length > 0 && userAddress) {
        // Processar contratos sequencialmente para evitar sobrecarga do RPC
        for (let i = 0; i < pratiqueContracts.length; i++) {
          const contract = pratiqueContracts[i];
          try {
            // Carregar dados em paralelo para o mesmo contrato
            await Promise.all([
              loadContractRewards(contract),
              loadContractStakeData(contract)
            ]);

            // Pequeno delay entre contratos para não sobrecarregar o provider
            if (i < pratiqueContracts.length - 1) {
              await new Promise(resolve => setTimeout(resolve, 100));
            }
          } catch (error) {
            console.error(`Error loading data for contract ${contract.address}:`, error);
            // Continuar com próximo contrato mesmo se este falhar
          }
        }
      }
    };
  ...
  const loadPratiqueContracts = async (forceRefresh = false) => {
    try {
      ...
      if (!userAddress) {
        ...
        if (onContractsLoaded) {
          onContractsLoaded(false);
        }
        return;
      }
      ...
      // Filtrar contratos do tipo 'pratique'
      const pratiqueOnly = (result.privateOffers || []).filter(contract =>
        contract.metadata?.investment_type === 'pratique'
      );
      ...
      setPratiqueContracts(authorizedContracts);
      ...
      // Notificar componente pai sobre quantidade de contratos
      if (onContractsLoaded) {
        onContractsLoaded(authorizedContracts.length > 0);
      }
    } catch (error) {
      console.error('Error loading Pratique contracts:', error);
      setPratiqueContracts([]);
      showError(t('pedacinhoPratique.errors.loadContracts'));

      // Notificar componente pai que não há contratos
      if (onContractsLoaded) {
        onContractsLoaded(false);
      }
    } finally {
  ...
  const fetchTokenInfo = async (tokenAddress, network) => {
    try {
      ...
      // Primeiro tentar via contract call (mais confiável) com retry
      const symbolResponse = await retryWithBackoff(() =>
  ...
  const loadContractRewards = async (contract) => {
    try {
      ...
      // Carregar pending rewards com retry
  ...
  const loadContractStakeData = async (contract) => {
    try {
      ...
      // Carregar saldo do usuário em stake com retry
      const userStakeResponse = await retryWithBackoff(() =>
        api.post('/api/contracts/read', {
          contractAddress: contract.address,
      ...
      // Carregar total staked supply com retry
      const totalStakedResponse = await retryWithBackoff(() =>
        api.post('/api/contracts/read', {
          contractAddress: contract.address,
      ...
      // Verificar se stake está bloqueado com retry
      const stakingBlockedResponse = await retryWithBackoff(() =>
        api.post('/api/contracts/read', {
          contractAddress: contract.address,
          functionName: 'stakingBlocked',
          params: [],
          network: contract.network
        })
      );
      ...
      if (stakingBlockedResponse.data.success && stakingBlockedResponse.data.data?.result !== undefined) {
        stakingBlocked = Boolean(stakingBlockedResponse.data.data.result);
      }
  ...
  const handleStatementsClick = (contract) => {
    setSelectedContract(contract);
    setStatementsModalOpen(true);
  };

  const handleBuyClick = () => {
    // Comprar PCN: cBRL em cima, PCN embaixo
    router.push('/exchange/market?from=cBRL&to=PCN');
  };

  const handleSellClick = () => {
    // Vender PCN: PCN em cima, cBRL embaixo
    router.push('/exchange/market?from=PCN&to=cBRL');
  };
  ...
  // Obter saldo PCN do cache (já carregado pelo useCachedBalances)
  const pcnBalance = getBalance('PCN') || '0';


  const getRiskBadge = (riskLevel) => {
    const riskConfig = {
      0: {
        label: t('pedacinhoPratique.risk.veryLow'),
        color: 'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400',
        icon: 'bi:reception-0'
      },
      1: {
        label: t('pedacinhoPratique.risk.low'),
        color: 'bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-400',
        icon: 'bi:reception-1'
      },
      2: {
        label: t('pedacinhoPratique.risk.medium'),
        color: 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-400',
        icon: 'bi:reception-2'
      },
      3: {
        label: t('pedacinhoPratique.risk.high'),
        color: 'bg-orange-100 text-orange-800 dark:bg-orange-900/20 dark:text-orange-400',
        icon: 'bi:reception-3'
      },
      4: {
        label: t('pedacinhoPratique.risk.veryHigh'),
    ...
    return (
      <Tooltip content={t('pedacinhoPratique.risk.label', { level: risk.label })} placement="top">
          ...
          <div className="flex items-center space-x-2">
            ...
            <div>
              <div className="text-xs">
                {t('pedacinhoPratique.pcnBalance')}
              </div>
        ...
        <div className="text-center py-12">
          <div className="flex flex-col items-center">
            ...
            <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">
              {t('pedacinhoPratique.empty.title')}
            </h3>
            <p className="text-gray-500 dark:text-gray-400">
              {t('pedacinhoPratique.empty.description')}
            </p>
          </div>
        </div>
      ) : (
              ...
              <div className="p-6">
                ...
                <div className="flex justify-between items-center mb-4">
                    ...
                    {loadingRewards[contract.address] ? (
                      ...
                      <>
                        ...
                        <div className="text-xs text-gray-600 dark:text-gray-400 uppercase mt-1">
                          {t('pedacinhoPratique.card.toReceive', { symbol: tokenInfo[contract.address]?.symbol || 'PCN' })}
                        </div>
                      </>
                    )}
                  </div>
                  ...
                  <div className="flex flex-col space-y-2">
                    ...
                    >
                      {t('pedacinhoPratique.card.claimButton')}
                    </button>
                    <button
                      disabled={!contractRewards[contract.address] || parseFloat(contractRewards[contract.address]) <= 0}
                    ...
                    >
                      {t('pedacinhoPratique.card.compoundButton')}
                    </button>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-4 gap-2 mb-2">
                  ...
                  >
                    {t('pedacinhoPratique.card.buyButton')}
                  </button>
                  <button
                    onClick={handleSellClick}
                    className="px-2 py-1.5 text-xs font-medium text-white bg-red-600 hover:bg-red-700 rounded-full cursor-pointer transition-colors"
                  >
                    {t('pedacinhoPratique.card.sellButton')}
                  </button>
                  {contractStakeData[contract.address]?.stakingBlocked ? (
                    <Tooltip
                      content={t('pedacinhoPratique.card.stakingBlocked')}
                      placement="top"
                      theme="dark"
                    >
                      <div className="w-full">
                        <button
                          disabled
                          className="w-full px-2 py-1.5 text-xs font-medium text-gray-400 bg-gray-300 rounded-full cursor-not-allowed transition-colors"
                        >
                          {t('pedacinhoPratique.card.stakeButton')}
                        </button>
                      </div>
                    </Tooltip>
                  ) : (
                  ...
                  <button
                    ...
                    className="px-2 py-1.5 text-xs font-medium text-white bg-red-600 hover:bg-red-700 rounded-full cursor-pointer transition-colors"
                  >
                    {t('pedacinhoPratique.card.unstakeButton')}
                  </button>
                </div>

                {/* Botão Demonstrativos */}
                <div className="mb-4">
                  <button
                    onClick={() => handleStatementsClick(contract)}
                    className="w-full px-2 py-1.5 text-xs font-medium text-red-600 border border-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-full cursor-pointer transition-colors"
                  >
                    {t('pedacinhoPratique.card.statementsButton')}
                  </button>
                </div>
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between items-center">
                    <span className="text-gray-600 dark:text-gray-400">{t('pedacinhoPratique.card.risk')}</span>
                    {getRiskBadge(contract.metadata?.risk ?? 1)}
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600 dark:text-gray-400">{t('pedacinhoPratique.card.lastQuarterly')}</span>
                    <span className="font-semibold text-gray-900 dark:text-white">
                      {contract.metadata?.lastDistribution || '--'}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600 dark:text-gray-400">{t('pedacinhoPratique.card.nextDue')}</span>
                    <span className="font-semibold text-gray-900 dark:text-white">
                      {formatDate(contract.metadata?.nextDistribution)}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600 dark:text-gray-400">{t('pedacinhoPratique.card.myStake')}</span>
                    <span className="font-semibold text-gray-900 dark:text-white">
                      {isLoadingStake ? (
                ...
                >
                  <span className="text-sm font-medium mr-1">{t('pedacinhoPratique.card.more')}</span>
                ...
                {expandedCards[index] && (
                  <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                    <div className="space-y-3">
                      {/* Total Staked Supply */}
                      <div className="flex justify-between">
                        <span className="text-gray-600 dark:text-gray-400">{t('pedacinhoPratique.card.totalStaked')}</span>
                        <span className="font-semibold text-gray-900 dark:text-white">
                          {isLoadingStake ? (
                      ...
                      <Tooltip
                        content={t('pedacinhoPratique.card.tooltipInfo')}
                        placement="top"
                      >
                        <div className="w-full flex items-center justify-center text-red-600 hover:text-red-700 transition-colors cursor-help mt-3">
                          ...
                          <span className="text-sm font-medium">{t('pedacinhoPratique.title')}</span>
                        </div>
                      </Tooltip>
                      {contract.hasError && (
                        <p className="text-xs text-red-600 dark:text-red-400 mt-1 text-center">
                          {t('pedacinhoPratique.card.errorLoading')}
                        </p>
                      )}
                    </div>
                  </div>
      ...
      {selectedContract && (
        <>
          ...
          <CompoundRewardsModal
            ...
            rewardsAmount={contractRewards[selectedContract.address] || '0'}
            tokenSymbol={tokenInfo[selectedContract.address]?.symbol || 'TOKEN'}
            totalEmission={selectedContract.metadata?.totalEmission}
            totalStaked={contractStakeData[selectedContract.address]?.totalStaked || '0'}
            onSuccess={handleModalSuccess}
          />
