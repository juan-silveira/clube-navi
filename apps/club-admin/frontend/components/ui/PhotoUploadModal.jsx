...
const PhotoUploadModal = ({ isOpen, onClose, onPhotoUploaded, currentPhoto }) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      // Usar o s3PhotoService que já tem toda a lógica implementada
      const result = await s3PhotoService.saveProfilePhoto(user.id, file);

      if (result.success) {
        showSuccess('Foto de perfil atualizada com sucesso!');
        
        // O s3PhotoService já disparou o evento e atualizou o cache
        // Só precisamos atualizar o store local
      ...
      showError(error.message || 'Erro ao enviar foto. Tente novamente.');
    } finally {
