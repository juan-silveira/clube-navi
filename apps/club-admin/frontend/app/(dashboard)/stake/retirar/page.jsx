...
const RetirarPage = () => {
  ...
  const loadStakeInfo = async () => {
    try {
      ...
      showError(t('messages.errorLoadingStake'));
    } finally {
  ...
  const handleUnstake = async () => {
    try {
      ...
      if (parseFloat(amount) > parseFloat(totalStakeBalance)) {
        showError(t('messages.amountExceedsStake'));
        return;
      }
      ...
      if (response.data.success) {
        showSuccess(t('messages.withdrawalSuccess'));
        router.push('/investments');
      } else {
        throw new Error(response.data.message || t('messages.errorWithdrawing'));
      }
    } catch (error) {
      console.error('Error unstaking:', error);
      showError(error.message || t('messages.errorWithdrawing'));
    } finally {
      setLoading(false);
    }
  };
