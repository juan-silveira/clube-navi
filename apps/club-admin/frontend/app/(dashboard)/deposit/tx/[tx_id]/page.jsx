...
const DepositConfirmationPage = () => {
  ...
  const fetchMintTransaction = useCallback(async (depositTxId) => {
    try {
      ...
      const response = await fetch(`/api/deposits/status/${depositTxId}`, {
        headers: {
        ...
        if (data.success) {
          if (data.data) {
            ...
            if (unifiedTransaction.blockchainStatus === 'confirmed') {
              ...
              // MOSTRAR SUCESSO mas NÃO redirecionar automaticamente - APENAS UMA VEZ
  ...
  }, [router, showSuccess, accessToken]);
  ...
  useEffect(() => {
    const fetchTransaction = async () => {
      ...
      try {
        ...
        try {
          const apiResponse = await fetch(`/api/deposits/status/${txId}`, {
            headers: {
              'Authorization': `Bearer ${accessToken || ''}`
            }
          });
    ...
    fetchTransaction();
  }, [txId, fetchMintTransaction, accessToken]);
  ...
  useEffect(() => {
    ...
    hasShownSuccessAlert.current = false; // Reset alerta ao iniciar novo polling
    ...
    const pollTransactionStatus = async () => {
      ...
      try {
        ...
        // Verificar novamente após a requisição se deve parar
        if (shouldStopPolling.current && pollingInterval.current) {
          clearInterval(pollingInterval.current);
          pollingInterval.current = null;
        }
      } catch (error) {
    ...
    return (
      <div className="min-h-screen py-8">
        <div className="max-w-2xl mx-auto px-4">
          ...
          {/* Informações Adicionais - só mostrar se ainda pendente E não confirmado */}
          {isPending && !(mintTransaction && mintTransaction.status === 'confirmed') && (
            <>
