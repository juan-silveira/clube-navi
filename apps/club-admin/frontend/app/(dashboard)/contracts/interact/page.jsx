...
export default function ContractInteractPage() {
  ...
  const getErrorTitle = (errorType) => {
    switch (errorType) {
      case 'timeout':
        return t('interact.errors.timeout');
      case 'missing_function':
        return t('interact.errors.missingFunction');
      case 'access_denied':
        return t('interact.errors.accessDenied');
      case 'call_exception':
        return t('interact.errors.callException');
      case 'network':
        return t('interact.errors.network');
      case 'gas':
        return t('interact.errors.gas');
      case 'admin':
        return t('interact.errors.admin');
      case 'revert':
        return t('interact.errors.revert');
      default:
        return t('interact.errors.general');
    }
  };
  ...
  const fetchContracts = async () => {
    try {
      ...
      showError(t('interact.errors.loadingError'));
    } finally {
      setLoading(false);
    }
  };

  // Filtrar contratos baseado na rede padrão
  const filteredContracts = useMemo(() => {
    if (!config.defaultNetwork) return contracts;
    
    // Filtrar contratos que correspondem à rede padrão
    return contracts.filter(contract => {
      // Se o contrato tem network definida, comparar com defaultNetwork
      if (contract.network) {
        return contract.network.toLowerCase() === config.defaultNetwork.toLowerCase();
      }
      // Se não tem network definida, assumir que é mainnet para compatibilidade
      return config.defaultNetwork.toLowerCase() === 'mainnet';
    });
  }, [contracts, config.defaultNetwork]);
  ...
  const updateContractABI = async (contractId) => {
    try {
      ...
      if (response.data.success) {
        showSuccess(t('interact.errors.updateSuccess'));
        // Reload contracts
      ...
      showError(t('interact.errors.updateError'));
    } finally {
  ...
  useEffect(() => {
    if (selectedContract) {
      ...
      if (selectedContract.abi) {
        ...
        showError(t('interact.errors.noAbi'));
  ...
  const parseContractABI = (abi) => {
    try {
      ...
      showError(t('interact.errors.processingError'));
    }
  };
  ...
  const formatOrderOutput = (output, functionName) => {
    // Formatação especial para buyOrders e sellOrders
    if (functionName === 'buyOrders') {
      const buyOrderLabels = [
        'id', 'buyer', 'amountTokenB', 'pricePerTokenB', 'remainingAmount', 'isActive', 'createdAt'
      ];

      if (Array.isArray(output)) {
        // Se é um array, formatar cada item em linha separada
        return output.map((item, index) => `[${index}]${buyOrderLabels[index] || 'field'}: ${item}`).join('\n');
      } else {
        // Se é uma string com vírgulas, quebrar linha após cada vírgula
        const str = output.toString();
        if (str.includes(',')) {
          return str.split(',').map((item, index) => `[${index}]${buyOrderLabels[index] || 'field'}: ${item.trim()}`).join('\n');
        }
      }
    }

    if (functionName === 'sellOrders') {
      const sellOrderLabels = [
        'id', 'seller', 'amountTokenB', 'pricePerTokenB', 'remainingAmount', 'isActive', 'createdAt'
      ];

      if (Array.isArray(output)) {
        // Se é um array, formatar cada item em linha separada
        return output.map((item, index) => `[${index}]${sellOrderLabels[index] || 'field'}: ${item}`).join('\n');
      } else {
        // Se é uma string com vírgulas, quebrar linha após cada vírgula
        const str = output.toString();
        if (str.includes(',')) {
          return str.split(',').map((item, index) => `[${index}]${sellOrderLabels[index] || 'field'}: ${item.trim()}`).join('\n');
        }
      }
    }

    return null; // Retorna null se não for uma função de ordem
  };

  const formatOutput = (output, outputType, functionName = '') => {
    try {
      // Primeiro verificar se é uma função de ordem que precisa formatação especial
      const orderFormatted = formatOrderOutput(output, functionName);
      if (orderFormatted !== null) {
  ...
  const executeReadFunction = async (func) => {
    ...
    try {
      ...
      const params = func.inputs.map((input, index) => {
        ...
        console.log(`📊 Processing READ param: ${input.name}, Type: ${input.type}, Value: ${value}, Function: ${func.name}`);

        // Handle different input types
        if (input.type.includes('uint') || input.type.includes('int')) {
          // Para funções que usam IDs, usar o valor literal (sem conversão)
          if (func.name === 'buyOrders' ||
              func.name === 'sellOrders' ||
              func.name === 'isBuyOrderActive' ||
              func.name === 'isSellOrderActive' ||
              func.name === 'cancelBuyOrder' ||
              func.name === 'cancelSellOrder' ||
              input.name === 'orderId' ||
              input.name === '_orderId' ||
              input.name === 'buyOrderId' ||
              input.name === 'sellOrderId') {
            console.log(`✅ Using literal value for ID: ${value || '0'}`);
            return value || '0';
          }
          // Converter para wei apenas se o parâmetro contém "amount" ou "Amount"
          if (input.name.toLowerCase().includes('amount')) {
            const weiValue = ethers.parseUnits(value || '0', 18).toString();
            console.log(`💰 Converting amount to wei: ${value} -> ${weiValue}`);
            return weiValue;
          }
          // Para outros valores uint/int, usar valor literal
          console.log(`📌 Returning literal value for ${input.type}: ${value || '0'}`);
          return value || '0';
        }
        if (input.type === 'bool') {
          return value === 'true';
      ...
      if (response.data.success) {
        ...
        showSuccess(t('interact.errors.executeSuccess', { functionName: func.name }));
      }
    } catch (error) {
      ...
      if (error.response?.data?.message) {
        ...
        if (errorMessage.includes('requires admin permissions')) {
          ...
          errorType = 'timeout';
          errorMessage = t('interact.errors.functionTimeout', { functionName: func.name });
        } else if (errorMessage.includes('Function call timeout')) {
          errorType = 'timeout';
          errorMessage = t('interact.errors.functionNotAvailable', { functionName: func.name });
        } else if (errorMessage.includes('CALL_EXCEPTION')) {
          errorType = 'call_exception';
          errorMessage = t('interact.errors.functionFailed', { functionName: func.name });
        } else if (errorMessage.includes('missing revert data')) {
          errorType = 'missing_function';
          errorMessage = t('interact.errors.functionNotFound', { functionName: func.name });
        } else if (errorMessage.includes('network')) {
          errorType = 'network';
          errorMessage = t('interact.errors.networkError', { functionName: func.name });
        }
      } else {
  ...
  const executeWriteFunction = async (func) => {
    ...
    try {
      ...
      const params = func.inputs.map((input, index) => {
        ...
        console.log(`🔍 Processando parâmetro: ${input.name}, Tipo: ${input.type}, Valor: ${value}, Função: ${func.name}`);

        // Handle different input types
        if (input.type.includes('uint') || input.type.includes('int')) {
          if (input.type.includes('256')) {
            // Para funções que usam IDs ou timestamps, usar o valor literal (sem conversão)
            if (func.name === 'distributeReward' ||
                func.name === 'updateInitialCycleStartTime' ||
                func.name === 'getBuyOrder' ||
                func.name === 'getSellOrder' ||
                func.name === 'isBuyOrderActive' ||
                func.name === 'isSellOrderActive' ||
                func.name === 'cancelBuyOrder' ||
                func.name === 'cancelSellOrder' ||
                input.name === 'orderId' ||
                input.name === 'customTimestamp' ||
                input.name === '_customTimestamp') {
              console.log(`✅ Usando valor literal para ${input.name}: ${value}`);
              return value || '0';
            }
            // Para funções de exchange, tratar valores específicos
            if (['createBuyOrder', 'createSellOrder', 'marketBuy', 'marketSell'].includes(func.name)) {
              // Converter para wei apenas parâmetros que contenham "amount" ou "Amount" no nome
              if (input.name.toLowerCase().includes('amount')) {
                const weiValue = ethers.parseUnits(value || '0', 18).toString();
                console.log(`💱 Exchange - Converting amount to wei: ${value} -> ${weiValue}`);
                return weiValue;
              }
              // IDs de ordem são números inteiros simples
              if (input.name === '_orderId' || input.name?.includes('OrderId') || input.name === 'orderId') {
                console.log(`🆔 Exchange - Order ID: ${value}`);
                return value || '0';
              }
            }
            if (func.name === 'matchOrders' && input.name?.includes('OrderIds')) {
              // Array de IDs - converter string separada por vírgulas em array
              const ids = value ? value.split(',').map(id => id.trim()).filter(id => id) : [];
              console.log(`🔗 Match Orders - IDs: ${JSON.stringify(ids)}`);
              return ids;
            }
            // Para parâmetros que contenham "amount" ou "Amount", converter para wei
            if (input.name.toLowerCase().includes('amount')) {
              const weiValue = ethers.parseUnits(value || '0', 18).toString();
              console.log(`💰 Converting amount to wei: ${value} -> ${weiValue}`);
              return weiValue;
            }
            // Para outros valores uint256, usar valor literal
            console.log(`📌 Using literal value for ${input.type}: ${value || '0'}`);
            return value || '0';
          }
          return value || '0';
        }
      ...
      console.log('📝 Params processados:', params);
      ...
      if (func.name === 'distributeReward') {
      ...
      } else if (func.name === 'updateInitialCycleStartTime') {
        // Função administrativa - usar endpoint direto com timeout otimizado
        apiUrl = '/api/contracts/write';
        requestBodyFinal = {
          ...requestBody,
          options: {
            ...requestBody.options,
            adminFunction: true, // Sinalizar que é função administrativa
            fastConfirmation: true
          }
        };
      } else if (['createBuyOrder', 'createSellOrder', 'marketBuy', 'marketSell', 'matchOrders'].includes(func.name)) {
        // Exchange functions - use specific exchange endpoints
        // NOTE: cancelBuyOrder/cancelSellOrder are handled by generic /api/contracts/write
        ...
        apiUrl = `/api/exchange/${exchangeActions[func.name]}`;
        requestBodyFinal = {
          contractAddress: selectedContract.address,
          params,
          network: selectedContract.network
      ...
      if (response.data.success) {
        ...
        showSuccess(t('interact.errors.transactionSuccess', { txHash: response.data.data.transactionHash }));
      }
    } catch (error) {
      ...
      if (error.response?.data?.message) {
        ...
        if (errorMessage.includes('timed out') || errorMessage.includes('timeout')) {
          errorType = 'timeout';
          if (func.name === 'updateInitialCycleStartTime') {
            errorMessage = `Função '${func.name}' levou mais que 60s para confirmar. A transação pode ter sido enviada - verifique na blockchain em alguns minutos.`;
          } else {
            errorMessage = t('interact.errors.functionTimeout', { functionName: func.name });
          }
        } else if (errorMessage.includes('Function call timeout')) {
          ...
          errorType = 'gas';
          errorMessage = t('interact.errors.insufficientGas', { functionName: func.name });
        } else if (errorMessage.includes('revert')) {
          errorType = 'revert';
          errorMessage = t('interact.errors.transactionReverted', { functionName: func.name });
        } else if (errorMessage.includes('network')) {
          errorType = 'network';
          errorMessage = t('interact.errors.networkError', { functionName: func.name });
        } else if (errorMessage.includes('Admin address not found')) {
          errorType = 'admin';
          errorMessage = t('interact.errors.adminNotFound');
        }
      } else {
        errorMessage = error.message || 'Erro desconhecido';
      }
          ...
          <div className="flex items-center gap-3">
            ...
            {func.inputs.length > 0 && (
              <span className="text-xs text-gray-600 dark:text-gray-400">
                ({func.inputs.length} {func.inputs.length > 1 ? t('interact.functions.params') : t('interact.functions.param')})
              </span>
            )}
          </div>
          {isExpanded ? (
          ...
          <div className="p-4 bg-gray-50 dark:bg-gray-800/50 space-y-4">
            ...
            {func.inputs.length > 0 && (
              <div className="space-y-3">
                <h4 className="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase">{t('interact.functions.parameters')}</h4>
                  ...
                  <div key={`${functionKey}-input-${index}`} className="space-y-1">
                    ...
                    <input
                      ...
                      placeholder={
                        func.name === 'distributeReward' && input.type.includes('uint256')
                          ? t('interact.placeholders.distributionPercentage')
                          : ['createBuyOrder', 'createSellOrder'].includes(func.name) && input.name === '_amount'
                          ? t('interact.placeholders.orderAmount')
                          : ['createBuyOrder', 'createSellOrder'].includes(func.name) && input.name === '_price'
                          ? t('interact.placeholders.orderPrice')
                          : ['marketBuy'].includes(func.name) && input.name === '_amount'
                          ? t('interact.placeholders.marketBuyAmount')
                          : ['marketSell'].includes(func.name) && input.name === '_amount'
                          ? t('interact.placeholders.marketSellAmount')
                          : ['marketBuy', 'marketSell'].includes(func.name) && input.name === '_minAmountOut'
                          ? t('interact.placeholders.minAmountOut')
                          : ['cancelBuyOrder', 'cancelSellOrder'].includes(func.name) && input.name === '_orderId'
                          ? t('interact.placeholders.orderId')
                          : func.name === 'matchOrders' && input.name === '_buyOrderIds'
                          ? t('interact.placeholders.buyOrderIds')
                          : func.name === 'matchOrders' && input.name === '_sellOrderIds'
                          ? t('interact.placeholders.sellOrderIds')
                          : input.type.includes('uint256') && input.name?.toLowerCase().includes('amount')
                          ? t('interact.placeholders.amountInWei')
                          : t('interact.placeholders.enterType', { type: input.type })
                      }
                    ...
                    {func.name === 'distributeReward' && input.type.includes('uint256') && (
                      <p className="text-xs text-green-600 dark:text-green-400 mt-1">
                        💡 {t('interact.tips.percentageFormat')}
                      </p>
                    )}
                    {func.name !== 'distributeReward' && input.type.includes('uint256') && input.name?.toLowerCase().includes('amount') && (
                      <p className="text-xs text-blue-600 dark:text-blue-400 mt-1">
                        💡 {t('interact.tips.amountConversion')}
                      </p>
                    )}
                    {['createBuyOrder', 'createSellOrder'].includes(func.name) && input.name === '_price' && (
                      <p className="text-xs text-green-600 dark:text-green-400 mt-1">
                        💡 {t('interact.tips.exchangePrice')}
                      </p>
                    )}
                    {['marketBuy', 'marketSell'].includes(func.name) && input.name === '_minAmountOut' && (
                      <p className="text-xs text-orange-600 dark:text-orange-400 mt-1">
                        💡 {t('interact.tips.slippageProtection')}
                      </p>
                    )}
                    {func.name === 'matchOrders' && input.name?.includes('OrderIds') && (
                      <p className="text-xs text-purple-600 dark:text-purple-400 mt-1">
                        💡 {t('interact.tips.matchingOrders')}
                      </p>
                    )}
                    {input.name === '_newStartTime' && func.name === 'updateInitialCycleStartTime' && (
                      <div className="mt-2 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-300 dark:border-blue-700 rounded">
                        <p className="text-xs text-blue-700 dark:text-blue-400 font-medium mb-2">⏰ {t('interact.timestampHelper.title')}:</p>
                        <div className="space-y-2">
                          ...
                          >
                            {t('interact.timestampHelper.useNow', { timestamp: Math.floor(Date.now() / 1000) })}
                          </button>
                          <p className="text-xs text-blue-600 dark:text-blue-400">
                            {t('interact.timestampHelper.current', { date: new Date().toLocaleString() })}
                          </p>
                        </div>
                      </div>
                    )}
            ...
            >
              {isExecuting ? (
                <>
                  ...
                  {t('interact.functions.executing')}
                </>
              ) : (
                <>
                  <Send className="w-4 h-4" />
                  {isWrite ? t('interact.functions.write.sendButton') : t('interact.functions.read.queryButton')}
                </>
              )}
            </button>
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div className="flex-1 space-y-2">
                    <p className="text-sm font-medium">
                      {result.success ? t('interact.results.success') : getErrorTitle(result.errorType)}
                    </p>
                      ...
                      <div className="text-xs font-mono bg-gray-100 dark:bg-gray-900 p-2 rounded overflow-x-auto">
                        {typeof result.data === 'object' && result.data.transactionHash ? (
                          <div className="space-y-1">
                            <div>
                              <span className="text-gray-600 dark:text-gray-400">{t('interact.results.txHash')}: </span>
                              <span className="text-blue-600 dark:text-blue-400">{result.data.transactionHash}</span>
                            </div>
                            {result.data.gasUsed && (
                              <div>
                                <span className="text-gray-600 dark:text-gray-400">{t('interact.results.gasUsed')}: </span>
                          ...
                          <pre className="text-green-600 dark:text-green-400 whitespace-pre-wrap">
                            {func.outputs.length > 0
                              ? formatOutput(result.data, func.outputs[0].type, func.name)
                      ...
                      <div className="space-y-2">
                        ...
                        {(result.errorType === 'timeout' || result.errorType === 'missing_function') && (
                          <div className="text-xs text-yellow-700 dark:text-yellow-400 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-300 dark:border-yellow-600 rounded p-2">
                            <p className="font-medium mb-1">💡 {t('interact.tips.title')}:</p>
                            <p>{t('interact.tips.functionNotImplemented')}</p>
                          </div>
                        )}
                        {result.errorType === 'access_denied' && (
                          <div className="text-xs text-red-700 dark:text-red-400 bg-red-50 dark:bg-red-900/20 border border-red-300 dark:border-red-600 rounded p-2">
                            <p className="font-medium mb-1">🔒 {t('interact.tips.restrictedAccess')}:</p>
                            <p>{t('interact.tips.adminRequired')}</p>
                          </div>
                        )}
                        {result.errorType === 'call_exception' && (
                          <div className="text-xs text-blue-700 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/20 border border-blue-300 dark:border-blue-600 rounded p-2">
                            <p className="font-medium mb-1">🔧 {t('interact.tips.suggestion')}:</p>
                            <p>{t('interact.tips.checkParameters')}</p>
                          </div>
                        )}
                        {result.errorType === 'network' && (
                          <div className="text-xs text-purple-700 dark:text-purple-400 bg-purple-50 dark:bg-purple-900/20 border border-purple-300 dark:border-purple-600 rounded p-2">
                            <p className="font-medium mb-1">🌐 {t('interact.tips.networkIssue')}:</p>
                            <p>{t('interact.tips.connectivityProblem')}</p>
                          </div>
                        )}
                      </div>
                    )}
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold flex items-center gap-2 text-gray-900 dark:text-white">
            ...
            {t('interact.contractSelector.title')}
          </h2>
        </div>
              ...
              >
                ...
                {contract.adminAddress && (
                  <div className="flex items-center gap-1 mt-2">
                    ...
                    <span className="text-xs text-gray-600 dark:text-gray-500">{t('interact.contractSelector.admin')}:</span>
                    <span className="text-xs text-amber-600 dark:text-amber-400 font-mono truncate">{contract.adminAddress}</span>
                  </div>
                )}
                <p className="text-xs text-gray-600 dark:text-gray-500 mt-2">
                  {t('interact.contractSelector.network')}: <span className="text-gray-500 dark:text-gray-400">{contract.network}</span>
                </p>
              </button>
              );
            })}
        ...
        {filteredContracts.length === 0 && !loading && (
          <div className="text-center py-8 text-gray-600 dark:text-gray-400">
            ...
            <p>
              {contracts.length > 0
                ? t('interact.contractSelector.noContractsForNetwork', { network: config.defaultNetwork || 'selecionada' })
                : t('interact.contractSelector.noContracts')}
            </p>
            {contracts.length > 0 && (
              <p className="text-sm mt-2">
                {t('interact.contractSelector.totalInOtherNetworks', { count: contracts.length })}
              </p>
            )}
          </div>
        )}
      ...
      {selectedContract && !selectedContract.abi && (
        <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-300 dark:border-yellow-600 rounded-lg p-6 text-center">
          ...
          <h3 className="text-lg font-semibold text-yellow-700 dark:text-yellow-400 mb-2">
            {t('interact.abiMissing.title')}
          </h3>
          <p className="text-gray-600 dark:text-gray-400 mb-4">
            {t('interact.abiMissing.description')}
          </p>
          <button
          ...
          >
            {updatingABI ? (
              <>
                <Loader2 className="w-4 h-4 animate-spin" />
                {t('interact.abiMissing.updating')}
              </>
            ) : (
              <>
                <Code2 className="w-4 h-4" />
                {t('interact.abiMissing.updateButton')}
              </>
            )}
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-6">
            <h2 className="text-lg font-semibold mb-4 flex items-center gap-2 text-gray-900 dark:text-white">
              <Eye className="w-5 h-5 text-blue-400" />
              {t('interact.functions.read.title')}
              <span className="text-xs text-gray-500 dark:text-gray-400">
                ({t('interact.functions.read.subtitle')})
              </span>
            </h2>
            <div className="space-y-3">
              {contractFunctions.read.length > 0 ? (
                ...
                <p className="text-gray-600 dark:text-gray-400 text-sm">{t('interact.functions.read.none')}</p>
              )}
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-6">
            <h2 className="text-lg font-semibold mb-4 flex items-center gap-2 text-gray-900 dark:text-white">
              <Zap className="w-5 h-5 text-orange-400" />
              {t('interact.functions.write.title')}
              <span className="text-xs text-gray-500 dark:text-gray-400">
                ({t('interact.functions.write.subtitle')})
              </span>
            </h2>
            <div className="space-y-3">
              {contractFunctions.write.length > 0 ? (
                ...
                <p className="text-gray-600 dark:text-gray-400 text-sm">{t('interact.functions.write.none')}</p>
              )}
            </div>
          </div>
        </div>
