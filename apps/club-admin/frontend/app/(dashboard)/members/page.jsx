...
const MembersPage = () => {
  ...
  const fetchMembers = async () => {
    try {
      setLoading(true);
      const response = await userService.getAll({ userType: 'consumer' });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h4 className="text-2xl font-bold text-slate-900 dark:text-white">Membros</h4>
          <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">
            {filteredMembers.length} de {stats.total} membros
          </p>
        </div>
        <button
        ...
        <Card>
          <div className="p-4">
            <div className="flex items-center justify-between">
              ...
              <Icon icon="heroicons:users" className="w-10 h-10 text-primary-500" />
            </div>
          </div>
        </Card>
        <Card>
      ...
      {/* Table */}
      <Card title={`Lista de Membros (${filteredMembers.length})`}>
              ...
              {filteredMembers.length === 0 ? (
                <tr>
                  <td colSpan="7" className="px-6 py-12 text-center">
                    <Icon icon="heroicons:users" className="w-12 h-12 text-slate-300 mx-auto mb-2" />
                    <p className="text-slate-500 dark:text-slate-400">
                      {members.length === 0 ? 'Nenhum membro cadastrado' : 'Nenhum resultado encontrado'}
                    </p>
                    {members.length > 0 && (
                      <button
                        onClick={() => {
                ...
                filteredMembers.map((member) => (
                  <tr key={member.id} className="hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-slate-600 dark:text-slate-400">@{member.username || '-'}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-slate-600 dark:text-slate-400">{member.email}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-slate-600 dark:text-slate-400">{member.cpf || '-'}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-slate-600 dark:text-slate-400">{member.phone || '-'}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
