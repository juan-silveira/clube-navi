...
const UserProfilePage = () => {
  ...
  // console.log("🚀 ADMIN USER PROFILE PAGE CARREGADO!!! - ARQUIVO CORRETO [ID]");
  ...
  const handleBlockUser = async () => {
    showInfo("Funcionalidade de bloqueio/desbloqueio será implementada em breve");
  };
  ...
  const handleToggleActive = async () => {
    try {
      ...
      if (response.success) {
        ...
        setEditData({ ...editData, isActive: !user.isActive });
        showSuccess(`Usuário ${!user.isActive ? 'ativado' : 'desativado'} com sucesso`);
      } else {
  ...
  const handlePromoteUser = async () => {
    const isCurrentlyAdmin = user.role === "ADMIN" || user.role === "APP_ADMIN" || user.role === "SUPER_ADMIN";
    const newRole = isCurrentlyAdmin ? "USER" : "ADMIN";

    try {
      setSaving(true);
      // Aqui você precisará implementar a lógica específica do seu backend
      // para alterar roles de usuário, que pode envolver userCompanies
      showInfo(`Funcionalidade de ${isCurrentlyAdmin ? 'remoção' : 'promoção'} de admin será implementada`);
    } catch (error) {
      console.error("Erro ao alterar role do usuário:", error);
      showError("Erro ao alterar privilégios do usuário");
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="flex items-center justify-between">
        ...
        {/* <div className="flex items-center space-x-3">
          ...
          <Badge
            ...
            className={`${getStatusBadgeColor(user.isActive)} text-white`}
          />
        </div> */}
      </div>
        ...
        <div className="p-6">
          ...
          {activeTab === "profile" && (
            <div className="space-y-6">
              {/* Botão Editar e Status no topo da aba */}
                  ...
                  <div className="flex items-center space-x-3">
                    {editMode && (
                      <Button
                        ...
                        isLoading={saving}
                        variant="primary"
                        className="flex items-center justify-center bg-primary-500 hover:bg-primary-600 text-white border-primary-500 hover:border-primary-600"
                      >
          ...
          {/* Tab: Documentos */}
          {activeTab === "documents" && <UserDocumentsTab userId={userId} />}
