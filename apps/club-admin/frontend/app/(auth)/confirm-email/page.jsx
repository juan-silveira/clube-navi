...
export default function ConfirmEmailPage() {
  ...
  const [userEmail, setUserEmail] = useState(null); // Email do usuário extraído do token
  const [showEmailInput, setShowEmailInput] = useState(false); // Mostrar campo de input de email
  const [emailInput, setEmailInput] = useState(''); // Email digitado pelo usuário
    ...
    const confirmEmail = async () => {
      ...
      try {
        ...
        if (response.ok && data.success) {
          ...
          // Extrair email do response para reenvio automático
          if (data.data?.email) {
            setUserEmail(data.data.email);
            console.log('📧 Email extraído do token:', data.data.email);
          }
        }
  ...
  const handleResendEmail = async () => {
    try {
      // Se não temos o email automaticamente, mostrar campo de input
      let emailToResend = userEmail || emailInput;

      if (!emailToResend) {
        setShowEmailInput(true);
        setMessage('Por favor, digite seu email para reenviar a confirmação');
        return;
      }
      ...
      >
        <div className="max-w-md w-full space-y-8">
          ...
          <Card className="mt-8 p-8 text-center">
            ...
            {status === 'error' && userEmail && !showEmailInput && (
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
                Clique no botão abaixo para reenviar a confirmação para: <strong>{userEmail}</strong>
              </p>
            )}
